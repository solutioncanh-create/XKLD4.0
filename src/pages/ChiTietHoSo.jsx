    ...
    return (
        ...
        <div className="bg-gray-50 min-h-screen font-sans">
            ...
            <div className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 z-50 flex items-center justify-between px-4 lg:px-8 shadow-sm">
                <Link to="/admin/ho-so" className="flex items-center gap-2 text-gray-500 hover:text-emerald-600 font-bold transition-colors group">
                    <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center group-hover:bg-emerald-100 group-hover:text-emerald-600 transition-colors">
                ...
                <div className="flex items-center gap-2">
                    <Link
                        to={`/sua-ho-so/${id}`}
                        className="px-3 py-1.5 bg-white border border-gray-200 text-gray-600 hover:bg-yellow-50 hover:text-yellow-600 hover:border-yellow-200 font-bold rounded-lg transition-all flex items-center gap-1.5 text-sm shadow-sm"
                    >
                    ...
                    {canPrint && (
                        <>
                            <button
                                ...
                                className="px-3 py-1.5 bg-white border border-gray-200 text-gray-600 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 font-bold rounded-lg transition-all flex items-center gap-1.5 text-sm shadow-sm"
                            >
                                <span className="material-icons-outlined text-lg">print</span>
                                <span className="hidden sm:inline">In HS</span>
                            </button>
                            <button
                                onClick={() => window.open(`/in-ho-so-jp/${id}`, '_blank')}
                                className="px-3 py-1.5 bg-white border border-gray-200 text-gray-600 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 font-bold rounded-lg transition-all flex items-center gap-1.5 text-sm shadow-sm"
                            >
                                <span className="material-icons-outlined text-lg">language</span>
                                <span className="hidden sm:inline">In Tiếng Nhật</span>
                            </button>
                        </>
                    )}
                    ...
                    <button
                        ...
                        className="px-3 py-1.5 bg-white border border-gray-200 text-gray-600 hover:bg-red-50 hover:text-red-600 hover:border-red-200 font-bold rounded-lg transition-all flex items-center gap-1.5 text-sm shadow-sm ml-2"
                    >
            ...
            <div className="max-w-5xl mx-auto px-4 py-8 pt-24 animate-fade-in-up">
                ...
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 animate-fade-in">
                    ...
                    {/* Ảnh CCCD - Section Riêng */}
                    <div className="space-y-6 flex flex-col items-center">
                        <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2 border-b-2 border-emerald-100 pb-2">
                            <span className="material-icons-outlined text-emerald-600 bg-emerald-50 p-1 rounded">badge</span>
                            Giấy Tờ Tùy Thân (CCCD/CMND)
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl">
                            <div className="space-y-3">
                                <div className="aspect-[1.58] rounded-xl border-2 border-white shadow-md overflow-hidden bg-gray-200 flex items-center justify-center relative group transition-transform hover:scale-[1.02]">
                                    {hoSo.anh_cccd_mat_truoc ? (
                                        <img src={hoSo.anh_cccd_mat_truoc} alt="Mặt trước" className="w-full h-full object-cover" />
                                    ) : (
                                        <div className="flex flex-col items-center text-gray-400">
                                            <span className="material-icons-outlined text-5xl mb-2">badge</span>
                                            <span className="text-sm font-medium">Mặt trước</span>
                                        </div>
                                    )}
                                    {hoSo.anh_cccd_mat_truoc && (
                                        <a href={hoSo.anh_cccd_mat_truoc} target="_blank" rel="noreferrer" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-sm">
                                            <span className="material-icons-outlined text-white text-3xl drop-shadow-lg">visibility</span>
                                        </a>
                                    )}
                                </div>
                                <p className="text-center text-sm font-bold text-gray-600 uppercase tracking-wide">Mặt trước</p>
                            </div>

                            <div className="space-y-3">
                                <div className="aspect-[1.58] rounded-xl border-2 border-white shadow-md overflow-hidden bg-gray-200 flex items-center justify-center relative group transition-transform hover:scale-[1.02]">
                                    {hoSo.anh_cccd_mat_sau ? (
                                        <img src={hoSo.anh_cccd_mat_sau} alt="Mặt sau" className="w-full h-full object-cover" />
                                    ) : (
                                        <div className="flex flex-col items-center text-gray-400">
                                            <span className="material-icons-outlined text-5xl mb-2">badge</span>
                                    ...
                                    {hoSo.anh_cccd_mat_sau && (
                                        <a href={hoSo.anh_cccd_mat_sau} target="_blank" rel="noreferrer" className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-sm">
                                            <span className="material-icons-outlined text-white text-3xl drop-shadow-lg">visibility</span>
                                        </a>
                                    )}
                                </div>
                                <p className="text-center text-sm font-bold text-gray-600 uppercase tracking-wide">Mặt sau</p>
                            </div>
                        </div>
                    </div>
                </div>
